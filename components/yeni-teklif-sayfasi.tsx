...
export async function YeniTeklifSayfasi({
  tip,
  firma_id,
}: {
  tip: "alis" | "satis";
  ...
  const [{ data: firmalar }, { data: urunler }, { data: ekip }, { data: sirket }] = await Promise.all([
    ...
    supabase
      .from("sirket_profili")
      .select("varsayilan_notlar, ozel_sablon_url, alis_teklif_sablon_url")
      .eq("id", true)
      .maybeSingle(),
  ]);
    ...
    <div>
      <div className="mb-6">
        <h1 className="font-display text-[22px] font-semibold">
          {tip === "alis" ? "Yeni Alış Teklifi" : "Yeni Satış Teklifi"}
        </h1>
      ...
      <TeklifOlusturForm
        ...
        sirketSablonVarMi={
          tip === "alis" ? !!sirket?.alis_teklif_sablon_url : !!sirket?.ozel_sablon_url
        }
        tip={tip}
      />
    </div>
