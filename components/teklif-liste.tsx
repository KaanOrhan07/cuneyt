...
import type { Firma, TeklifDurum } from "@/lib/types";

export type TeklifFiltreleri = {
  firma_id?: string;
  durum?: string;
};

export async function TeklifListe({
  tip,
  filters,
}: {
  tip: "alis" | "satis";
  filters: TeklifFiltreleri;
}) {
  const temelYol = tip === "alis" ? "/alis-teklifleri" : "/teklifler";
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="font-display text-[22px] font-semibold">
            {tip === "alis" ? "Alış Teklifleri" : "Satış Teklifleri"}
          </h1>
          <p className="mt-0.5 text-[13px] text-text-dim">
            {tip === "alis"
              ? "Tedarikçilerden alınan teklifler ve alış teklifi formları"
              : "Müşterilere verilen teklifler"}
          </p>
        </div>
        <Link href={`${temelYol}/yeni`}>
      ...
      <form className="mb-5 flex flex-wrap gap-2.5">
        <Select name="firma_id" defaultValue={filters.firma_id ?? ""}>
          <option value="">{tip === "alis" ? "Tüm Tedarikçiler" : "Tüm Firmalar"}</option>
            ...
            {teklifler?.map((t) => {
              ...
              return (
                <tr key={t.id} className="border-b border-border last:border-0">
                  <td className="py-2.5">
                    <Link href={`${temelYol}/${t.id}`} className="font-medium text-green hover:underline">
                      {t.teklif_no || "Detay →"}
                    </Link>
                  </td>
